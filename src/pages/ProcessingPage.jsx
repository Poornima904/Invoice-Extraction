...
export default function ProcessingPage() {
  ...
  return (
    <div className="max-w-10xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-gray-900 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 gap-3 sm:gap-0">
        ...
        <div className="flex gap-3 flex-wrap">
          <button className="flex items-center gap-2 px-4 py-2 border border-blue-600 bg-blue-100 text-blue-600 rounded-md font-semibold text-sm">
            <FaSpinner className="animate-spin" /> Processing
          </button>
          <button className="flex items-center gap-2 px-4 py-2 border rounded-md font-semibold text-sm hover:bg-gray-50">
      ...
      <div className="border rounded-xl p-5 mb-5">
        <div className="flex items-center gap-2 mb-4 text-lg font-semibold">
          <FaClock className="text-blue-600" /> Processing Timeline
        </div>
        ...
        <div className="w-full h-2 bg-gray-200 rounded-full mb-5 overflow-hidden">
          <div className="h-full bg-gray-900 rounded-full transition-all duration-300" style={{ width: `${overallProgress}%` }} />
        </div>

        {/* Steps */}
        <div className="relative pl-14 sm:pl-16">
          {/* Vertical Line */}
          <div className="absolute top-5 left-7 sm:left-8 w-0.5 bg-gray-300 h-full z-0"></div>
          ...
          {stepsData.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center mb-8 relative gap-2 sm:gap-4">
                
                {/* Icon */}
                <div className="flex-shrink-0 w-14 flex justify-center mt-1">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center z-10
                    ${isCompleted ? "bg-green-500 text-white" : isProcessing ? "bg-blue-600 text-white" : "bg-gray-300 text-white"}`}>
                    {isCompleted ? <FaCheck /> : isProcessing ? <FaSpinner className="animate-spin" /> : <div className="w-2.5 h-2.5 rounded-full bg-white"></div>}
                  </div>
                </div>
                ...
                <div className="flex-1 pl-4">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
                    <div className={`text-sm font-semibold ${isCompleted ? "text-green-600" : isProcessing ? "text-blue-600" : "text-gray-900"}`}>
                      {step.title}
                    </div>
                    {step.duration && (
                      <div className={`ml-auto text-xs px-2 py-0.5 border rounded ${isProcessing ? "border-blue-600 text-blue-600" : "border-gray-200 text-gray-700"}`}>
                        {step.duration}
                      </div>
                    )}
                  </div>
                  <div className={`mt-1 text-xs ${isProcessing ? "text-blue-600" : "text-gray-600"}`}>
                    {step.description}
                  </div>
                  ...
                  {isProcessing && step.progress !== undefined && (
                    <div className="mt-2 w-full h-1.5 bg-gray-200 rounded overflow-hidden">
                      <div className="h-full bg-black rounded transition-all duration-300" style={{ width: `${step.progress}%` }}></div>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-5 mb-5">
        <div className="flex-1 border rounded-xl p-5 min-w-[280px]">
        ...
        <div className="flex-1 border rounded-xl p-5 min-w-[280px]">
          ...
          <div className="flex justify-between mb-2"><span>Confidence Score:</span> <span className="px-2 py-1 bg-gray-100 rounded font-semibold">89%</span></div>
      ...
      <div className="flex justify-start">
        <button className="px-5 py-2 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 transition-colors">
